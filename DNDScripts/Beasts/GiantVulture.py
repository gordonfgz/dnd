...
def getMultiAttack(isAdv, count, CRITICAL_HIT_TEXT, AUTOMATION_COLOUR):
    ...
    critDamage1 = CritDamageCalculator.calculateCritDamage(2, DiceRolls.rollD, 4)
    damageDice2 = DiceRolls.rollD(6) + DiceRolls.rollD(6) + 2
    critDamage2 = CritDamageCalculator.calculateCritDamage(2, DiceRolls.rollD, 6)
    # first attack
    if (hitDice1 == 20):
    ...
    print(f"{AUTOMATION_COLOUR}G.Vulture {count} rolled {Colors.YELLOW}{hitDice1}{AUTOMATION_COLOUR} for {Colors.LIGHT_RED}{damageDice1} {damageType1} damage{AUTOMATION_COLOUR} and {Colors.YELLOW}{hitDice2}{AUTOMATION_COLOUR} for {Colors.LIGHT_RED}{damageDice2} {damageType2} damage{Colors.RESET}")